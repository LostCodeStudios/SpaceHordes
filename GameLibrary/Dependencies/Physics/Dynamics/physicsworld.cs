...
namespace GameLibrary.Dependencies.Physics.Dynamics
{
    ...
    public abstract class FilterData
    {
        ...
        public virtual bool IsActiveOn(PhysicsBody body)
        {
    ...
    public class PhysicsWorld
    {
        ...
        private PhysicsBody[] _stack = new PhysicsBody[64];
        private bool _stepComplete;
        private HashSet<PhysicsBody> _bodyAddList = new HashSet<PhysicsBody>();
        private HashSet<PhysicsBody> _bodyRemoveList = new HashSet<PhysicsBody>();
        private HashSet<PhysicsJoint> _jointAddList = new HashSet<PhysicsJoint>();
        private HashSet<PhysicsJoint> _jointRemoveList = new HashSet<PhysicsJoint>();
        ...
        public PhysicsWorld(Vector2 gravity, AABB span)
            : this()
        {
            Gravity = gravity;
        ...
        public List<PhysicsBreakableBody> BreakableBodyList { get; private set; }
        ...
        private void ProcessRemovedJoints()
        {
            ...
            {
                foreach (PhysicsJoint joint in _jointRemoveList)
                {
        ...
        private void ProcessAddedJoints()
        {
            ...
            {
                foreach (PhysicsJoint joint in _jointAddList)
                {
        ...
        private void Solve(ref TimeStep step)
        {
            ...
            foreach (PhysicsJoint j in JointList)
            {
                j.IslandFlag = false;
            }
            ...
            if (stackSize > _stack.Length)
                _stack = new PhysicsBody[Math.Max(_stack.Length * 2, stackSize)];

            for (int index = BodyList.Count - 1; index >= 0; index--)
            {
                PhysicsBody seed = BodyList[index];
                ...
                while (stackCount > 0)
                {
                    ...
                    PhysicsBody b = _stack[--stackCount];
        ...
        private void SolveTOI(ref TimeStep step)
        {
            ...
            for (; ; )
            {
                ...
                PhysicsBody bA1 = fA1.Body;
                PhysicsBody bB1 = fB1.Body;
                ...
                for (int i = 0; i < Island.BodyCount; ++i)
                {
                    PhysicsBody body = Island.Bodies[i];
        ...
        public void AddBreakableBody(PhysicsBreakableBody breakableBody)
        {
            BreakableBodyList.Add(breakableBody);
        }

        public void RemoveBreakableBody(PhysicsBreakableBody breakableBody)
        {
