...
using GameLibrary.Dependencies.Physics.Common.PhysicsLogic;
...
namespace GameLibrary.Dependencies.Physics.Dynamics
{
    ...
    public class PhysicsBody : IDisposable
    {
        ...
        public PhysicsBody DeepClone()
        {
            PhysicsBody body = Clone();
        ...
        /// <returns></returns>
        internal bool ShouldCollide(PhysicsBody other)
        {
        ...
        public void RestoreCollisionWith(PhysicsBody other)
        {
            for (int i = 0; i < FixtureList.Count; i++)
            {
                Fixture f = FixtureList[i];
